...
band of DM testing on this sample. At the 5-day horizon, XGBoost is
the only model with a statistically significant result---and it is
significantly \emph{worse} than HAR. The findings confirm the
well-known difficulty of beating HAR-RV in a market that has not
previously received a systematic deep-learning benchmark, and they
highlight the importance of using a single loss function consistently
for both evaluation and significance testing.
\end{abstract}
...
\citep{audrino2020machine, christensen2023machine}. The picture from
that literature is mixed: some studies report improvements from
boosted trees or recurrent networks on individual stock or index
volatility, while \citet{christensen2023machine} -- in the most
comprehensive recent benchmark on US data -- find that machine
learning rarely improves on a well-specified HAR-RV.
...
\begin{enumerate}
  ...
  tests at any horizon}. Numerical QLIKE differences exist but lie
  within the noise band of DM testing on this sample. The result
  confirms the well-known difficulty of beating HAR-RV, now
  documented systematically on an Indian-market dataset, and joins
  recent careful negative-result benchmarks
  \citep{christensen2023machine}.
\end{enumerate}
...
\paragraph{Should we conclude that ``ML cannot beat HAR-RV'' on
Indian markets?}
The honest answer is that, on this particular sample and with these
particular model specifications, we lack the statistical power to
distinguish the LSTM and Transformer from HAR at $h=1$, or GARCH
from HAR at $h=22$. The numerical gaps are economically suggestive
in directions consistent with intuition: sequence models exploit
short-horizon dependencies; GARCH-style mean-reverting forecasts
become more competitive at longer horizons where the conditional
mean of variance dominates the prediction. But these directions
should be re-tested on multi-asset panels or longer samples before
